...
- `app.py`: quote view and Option Chain Explorer. The explorer retrieves expirations, then displays the selected chain's strike, type, bid, ask, calculated mid price, strike distance, delta, implied volatility, volume, and open interest. Ticker Diagnostics aggregates the current chain's contract-quality results, including failure counts and primary strengths/weaknesses, without filtering, scoring, ranking, or recommendations. Its diagnostic control exposes the raw chain response for validation.
